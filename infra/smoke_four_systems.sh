...
BASE_URL="${1%/}"
API_KEY="$2"
SESSION_ID="three-lines-$(date +%Y%m%d%H%M%S)"
TODAY="$(date +%Y-%m-%d)"
TMP_DIR="$(mktemp -d)"
...
echo "== base readiness and three canonical lines =="
...
assert_json "$TMP_DIR/projects.json" "data.get('ok') is True and {'buyer_ai', 'commerce', 'xau'}.issubset({item.get('normalized_project_id') or item.get('memory_key') or item.get('content', {}).get('project_id') for item in data.get('items', [])})"
echo

echo "== task board three canonical lines =="
for lane in buyer_ai commerce xau; do
...
assert_json "$TMP_DIR/tasks.json" "data.get('ok') is True and {'buyer_ai', 'commerce', 'xau'}.issubset({item.get('content', {}).get('lane') for item in data.get('items', [])})"
echo

echo "== buyer-ai report e2e =="
curl ${curl_opts+"${curl_opts[@]}"} -fsS -X POST "${BASE_URL}/reports/create" \
  "${json_header[@]}" "${auth_header[@]}" \
  -d "{\"period\":\"daily\",\"date\":\"${TODAY}\"}" | tee "$TMP_DIR/report-create.json"
assert_json "$TMP_DIR/report-create.json" "data.get('ok') is True and data.get('report', {}).get('project_id') == 'buyer_ai' and data.get('report', {}).get('report_id')"
...
echo "== buyer-ai refund/recall e2e =="
curl ${curl_opts+"${curl_opts[@]}"} -fsS -X POST "${BASE_URL}/agents/run" \
  "${json_header[@]}" "${auth_header[@]}" \
  -d "{\"prompt\":\"退款 991\",\"session_id\":\"${SESSION_ID}-buyer-ai\",\"user_id\":\"smoke-buyer-ai\"}" | tee "$TMP_DIR/buyer-ai-refund.json"
assert_json "$TMP_DIR/buyer-ai-refund.json" "data.get('ok') is True and '991' in data.get('reply', '')"
curl ${curl_opts+"${curl_opts[@]}"} -fsS -X POST "${BASE_URL}/agents/run" \
  "${json_header[@]}" "${auth_header[@]}" \
  -d "{\"prompt\":\"991 點？\",\"session_id\":\"${SESSION_ID}-buyer-ai\",\"user_id\":\"smoke-buyer-ai\"}" | tee "$TMP_DIR/buyer-ai-recall.json"
assert_json "$TMP_DIR/buyer-ai-recall.json" "data.get('ok') is True and '991' in data.get('reply', '') and '沒有找到' not in data.get('reply', '')"
echo
...
echo "== buyer-ai dispatch/run_all e2e =="
curl ${curl_opts+"${curl_opts[@]}"} -fsS -X POST "${BASE_URL}/tasks/dispatch_plan" \
  "${json_header[@]}" "${auth_header[@]}" \
  -d "{\"project\":\"buyer_ai\",\"task_type\":\"code\",\"title\":\"Smoke AI Team run_all\",\"prompt\":\"fix repo bug and record routing\",\"session_id\":\"${SESSION_ID}-ai\",\"max_steps\":2}" | tee "$TMP_DIR/ai-plan.json"
assert_json "$TMP_DIR/ai-plan.json" "data.get('ok') is True and data.get('task_id') and data.get('plan', {}).get('project') == 'buyer_ai'"
...
curl ${curl_opts+"${curl_opts[@]}"} -fsS -X POST "${BASE_URL}/memory/timeline" \
  "${json_header[@]}" "${auth_header[@]}" \
  -d "{\"project_id\":\"buyer_ai\",\"session_id\":\"${SESSION_ID}-ai\",\"limit\":50}" | tee "$TMP_DIR/ai-timeline.json"
...
echo "Three canonical lines smoke OK: ${SESSION_ID}"
