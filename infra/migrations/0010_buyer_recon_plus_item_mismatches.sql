-- 0010_buyer_recon.sql + item_mismatches supplement
...
-- ─────────────────────────────────────────────────────────────
-- Per-SKU item mismatches (normalized — optional; also available as JSONB on item_comparisons.mismatched_items)
-- ─────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS item_mismatches (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    mismatch_id TEXT UNIQUE NOT NULL,
    comparison_id TEXT NOT NULL REFERENCES item_comparisons(comparison_id) ON DELETE CASCADE,
    sku TEXT,
    declared_qty INTEGER,
    scanned_qty INTEGER,
    declared_price INTEGER,
    scanned_price INTEGER,
    reason TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_item_mismatches_comparison_id ON item_mismatches(comparison_id);
